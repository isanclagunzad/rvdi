...
class GenerateSalarySheet extends Controller
{
        ...
        try {
            ...
            foreach ($BulkEmployee as $employeeDetails) {
                ...
                if (!empty($employeeDetails->pay_grade_id) || $employeeDetails->pay_grade_id != 0) {
                    ...
                    $allowance = $this->payrollRepository->calculateEmployeeAllowance(
                        $employeeDetails->payGrade->basic_salary, 
                        $employeeDetails->pay_grade_id
                    );

                    $deduction = $this->payrollRepository->calculateEmployeeDeduction(
                        $employeeDetails->payGrade->basic_salary, 
                        $employeeDetails->pay_grade_id
                    );
                    ...
                    $employeeAllInfo = $this->payrollRepository->getEmployeeWorkDetails(
    ...
    {
        $query = DB::select("SELECT temp.* FROM (
            SELECT DATE_FORMAT(date,'%Y-%m') AS yearAndMonth,view_employee_in_out_data.finger_print_id,employee.employee_id 
        ...
        $queryResult = SalaryDetails::where('employee_id', $request->employee_id)
                                    ->where('month_of_salary', $request->month)
                                    ->count();
