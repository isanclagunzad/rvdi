...
class EmployeePermanentController extends Controller
{
    ...
    public function index(Request $request){
        ...
        )->orderBy('date_of_joining','ASC')->where('status',UserStatus::$ACTIVE)->paginate(10);

        if (request()->ajax()) {
            if($request->role_id !='') {
                $results = Employee::whereHas('user', function($q) use($request){
                    $q->with('role')->where('role_id', $request->role_id);
                })->with('department', 'designation', 'branch', 'payGrade', 'supervisor', 'hourlySalaries')->where('status',UserStatus::$ACTIVE)->orderBy('date_of_joining','ASC');
            }else{
                $results = Employee::with(['user' => function ($q) {
                    $q->with('role');
                }, 'department', 'designation', 'branch', 'payGrade', 'supervisor', 'hourlySalaries'])->where('status',UserStatus::$ACTIVE)->orderBy('date_of_joining','ASC');
            }
