...
class PayrollRepository
{
     ...
     *
     */
    // WIP: Update code to comply with TRAIN Law
    public function calculateEmployeeTax($basic_salary, $deductions, $gender)
    {
        $taxableIncome      = $basic_salary - $deductions;
        $yearlyTaxableGross = $taxableIncome * 12;
        $genderTaxRule = TaxRule::whereGender($gender)->get();

        $yearlyTax = 0;
        $monthlyTax = 0;

        foreach ($genderTaxRule as $taxRule) {
            if(
                $taxRule->min_income_bracket >= $yearlyTaxableGross 
                && $taxRule->max_income_bracket <= $yearlyTaxableGross
            ) {
                if($taxRule->min_income_bracket == 0) {
                    $yearlyTax = 0;

                    continue;
                }

                $excessAfterMinimum = $yearlyTaxableGross - ($taxRule->min_income_bracket - 1);
                $yearlyTax = (($excessAfterMinimum * $taxRule->tax) / 100) + $taxRule->base_amount;
            }
        }
    ...
    public function getEmployeeWorkDetails($employee_id, $month, $overtime_rate, $basic_salary)
    {
