...
use App\Lib\Enumerations\UserGender;
...
class EmployeeRepository
{
    ...
    public function makeEmployeePersonalInformationDataFormat($data){
        ...
        $employeeData['pay_grade_type_id']   = $data['pay_grade_type_id'];
        ...
        $employeeData['suffix']         = $data['suffix'];
    ...
    {
        $query = Employee::query()->whereStatus(UserStatus::$ACTIVE);

        if($request->has('employee')) {
            $query->where(function($q) use ($request) {
                $exploded = explode(' ', $request->input('employee'));

                $f = $q->orWhere('first_name', 'like', '%'. $exploded[0] . '%')
                        ->orWhere('last_name', 'like', '%'. $exploded[0] . '%');
                
                foreach ($exploded as $key => $value) {
                    if($key > 0) {
                        $f->orWhere('first_name', 'like', '%'. $value . '%')
        ...
        if($request->has('gender')) {
            ...
            if(strtolower($gender) == UserGender::$MALE || strtolower($gender) == UserGender::$FEMALE) {
        ...
        if( $request->has('designation')
            ...
            $parameters = array_only($request->all(), ['designation', 'branch', 'department', 'role']);
            ...
            foreach ($parameters as $name => $value) {
                ...
                $query->whereHas($name == 'role' ? 'user.role' : $name, function($q) use($parameterName, $parameterValue) {
                    $q->where($parameterName . '_name', 'like', '%' . $parameterValue . '%');
                });
            }
        }
