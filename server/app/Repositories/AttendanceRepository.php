...
{
    public function getAttendanceFromBetweenDates($fromDate = '', $toDate = '') 
    {
        $formattedFromDate = $this->santiseDate($fromDate);
        $formattedToDate = $this->santiseDate($toDate);

        $attendance_list = DB::select('CALL SP_attendanceForGivenDates(?, ?)', array($formattedFromDate, $formattedToDate));
        $total_employees_count = Employee::whereStatus(1)->count();

        return compact('attendance_list', 'total_employees_count');
    }

    private function santiseDate($date)
    {
        if($date) {
            return dateConvertFormtoDB($date);
        }
    ...
    public function getEmployeeDailyAttendanceFromRequest(Request $request) 
    {
        $query = (new EmployeeRepository)->advancedSearch($request)->with('department', 'workshift');

        if(!$request->has('start_date') && !$request->has('end_date')) {
            $queryResult = $query->with([
                'inOutData' => function($query) {
        ...
        $queryResult = $query->with([
            'inOutData' => function($query) use ($startDate, $endDate) {
                $query->whereBetween('date', [dateConvertFormtoDB($startDate), dateConvertFormtoDB($endDate)]);
        }])->get();
